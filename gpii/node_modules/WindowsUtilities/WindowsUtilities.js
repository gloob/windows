...
var ref = require("ref");
var refWChar = require("ref-wchar");
...
/**
 * Contains flags used in the "fWinIni" paramater of the function SystemParametersInfo.
 * https://msdn.microsoft.com/en-us/library/windows/desktop/ms724947(v=vs.85).aspx
 */
windows.fWinIniFlags = {
 ...
 * @param {Array} allFlags [optional] An array of all possible flags and their value. Defaults to windows.flagConstants.
 ...
 * "|" or "&" specifying the operation, or an array of flag strings (in which case the operation is binary OR).
 *
 * The value of each possible flag is specified in allFlags.
 *
 * @param {String|Array} flags The list of flags to resolve.
 * @param {Array} allFlags An associative array of every flag.
 * @return {number} The numeric value of flags.
 */
windows.resolveFlags = function (flags, allFlags) {
    var togo = 0;
    if (!isNaN(Number(flags))) {
        // Use the hard-coded number as-is
        togo = Number(flags);
    } else if (flags) {
        var op = "or";
...
windows.arrayToBuffer = function (array, type) {
    ...
    if (type === "TCHAR") {
        for (i = 0; i < array.length; ++i) {
            refWChar.set(buf, i * size, array[i]);
        }
    } else {
        for (i = 0; i < array.length; ++i) {
...
windows.bufferToArray = function (buffer, type) {
    ...
    if (type === "TCHAR") {
        for (i = 0; i < buffer.length / size; ++i) {
            array[i] = refWChar.get(buffer, i * size);
        }
    } else {
        for (i = 0; i < buffer.length / size; ++i) {
